	...
	<div class="insuredInfo">
		...
		<div class="detail">
			<div class="info lastInfo">
				...
				<p class="placeholders">
					<input type="text" id='tianInsuredAddress'placeholder="请填写地址" class="placeholdersInput" v-model='addressValue'/>
				</p>
			</div>
			<div id='tianInsuredArea'></div>
		</div>
	</div>
</template>
...
//          this.choiceResidence()
        ...
        methods: {
           ...mapMutations({
            ...
            setDefault () {
               let name = this.tianCarInsuredAndApplicant.insuredInfo.name
               let card = this.tianCarInsuredAndApplicant.insuredInfo.cardNo
               let phone = this.tianCarInsuredAndApplicant.insuredInfo.phone
               let address = this.tianCarInsuredAndApplicant.insuredInfo.address
               let birthday = this.tianCarInsuredAndApplicant.insuredInfo.birthday
               this.nameValue = name
               this.cardValue = card
               this.phoneValue = phone
               this.birthdayValue = birthday
               this.addressValue = address
               this.gouStatus = this.tianCarInsuredAndApplicant.gouStatus
               console.log(this.gouStatus)
               console.log('this.gouStatus')
               this.circleStatus = this.tianCarInsuredAndApplicant.circleStatus
               console.log(this.circleStatus)
               console.log('this.circleStatus')
               console.log('default')
               if (this.nameValue === '') {
                ...
                if (this.tianCarInsuredAndApplicant.applicantInfo.sex) {
                    if (this.tianCarInsuredAndApplicant.insuredInfo.sex === '1') {
                        this.isStatusA = true
                        this.isStatusB = false
                    } else if (this.tianCarInsuredAndApplicant.insuredInfo.sex === '2') {
            ...
            changeCircle () {
                ...
                this.setTianCarInsuredAndApplicant({
                    field1: 'insuredInfo',
                    field2: 'name',
                    value: this.tianCarInsuredAndApplicant.applicantInfo.name
                })
                this.setTianCarInsuredAndApplicant({
                    field1: 'insuredInfo',
                    field2: 'cardNo',
                    value: this.tianCarInsuredAndApplicant.applicantInfo.cardNo
                })
                this.setTianCarInsuredAndApplicant({
                        field1: 'insuredInfo',
                        field2: 'phone',
                        value: this.tianCarInsuredAndApplicant.applicantInfo.phone
                    })
                this.setTianCarInsuredAndApplicant({
                    field1: 'insuredInfo',
                    field2: 'address',
                    value: this.tianCarInsuredAndApplicant.applicantInfo.address
                })
                this.setTianCarInsuredAndApplicant({
                    field1: 'insuredInfo',
                    field2: 'birthday',
                    value: this.tianCarInsuredAndApplicant.applicantInfo.birthday
                })
                this.setTianCarInsuredAndApplicant({
                    field1: 'insuredInfo',
                    field2: 'sex',
                    value: this.tianCarInsuredAndApplicant.applicantInfo.sex
                })
                this.setDefault()
            },
            verName () {
           ...
           verCard () {
            ...
            choiceResidence () {
            let vm = this
                linkage('tianInsuredAddress', 'tianInsuredArea', vm.pcArray, function (arr) {
                    let str = ''
                    for (let obj of arr) {
                        str += obj.value + ' '
                    }
