			...
			<div class="content">
				...
				<div class="detail" v-for="(item, index) in carArray">
					...
					<div class="carDetailWarp">
						<p class="carDetail"><span class="carName">车型名称：</span><span class="detailInfo">{{item.vehicleJingyouDto.vehicleName}}</span>
							<span :class="index === Ind ? 'redGou' : 'redCircle'" @click="changeCircle(index)"></span>
						</p>
						<p class="carDetail">车款：<span class="detailInfo">{{item.marketDate}}</span></p>
						<p class="carDetail">新车购置价：<span class="detailInfo">¥ {{item.purchasePrice}}</span></p>
						<p class="carDetail"><span class="carName">新车购置价（含税）：</span><span class="detailInfoPrice">¥ {{item.purchasePrice}}</span></p>
						<p class="carDetail"><span class="carName">车型信息：</span><span class="detailInfo">{{item.carName}}</span></p>
					</div>
				</div>
			</div>
...
<script>
	...
	import {confirmCarModel, queryCarModel} from 'api/car/tian-an/insurance.js'
	...
	export default {
        ...
        computed: {
            ...mapGetters([
                'tianCarConfig', 'tianCarPremium', 'tianCarInfo'
            ])
        },
        created () {
            console.log(this.tianCarConfig)
			this._queryCar()
        },
        methods: {
            ...mapMutations({
                    ...
                    console.log(res)
                        if (res.ret === RET_OK) {
                            if (res.data.code === CODE_OK) {
                                ...
                                if (res.data.vehicleModelList.length !== 0) {
                                    this.carArray = res.data.vehicleModelList
                                } else {
            ...
            _next () {
				...
				if (status === true) {
					...
					this.setPremium({
					field1: 'seatCounts',
					value: this.carArray[Num].seatCount
					})
					console.log(this.tianCarPremium)
					let vehicleCode = this.carArray[Num].vehicleJingyouDto.vehicleCode
