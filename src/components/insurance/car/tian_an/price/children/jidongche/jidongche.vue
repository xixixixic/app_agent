...
export default {
    ...
    methods: {
        ...
        yesStatus (index) {
            ...
            if (value === '投保') {
                this.Ind = index
                let values = this.status[index].value
                if (values === '是') {
                    this.setPremium({
                        field1: 'ItemKindRequestDto#14',
        ...
        setValue () {
            if (this.tianCarPremium.ItemKindRequestDto[0]) {
                    this.jidongcheValue = this.tianCarPremium.ItemKindRequestDto[0].amount === '0' ? '投保' : '不投保'
            }
            if (this.tianCarPremium.ItemKindRequestDto[14]) {
                    this.Ind = this.tianCarPremium.ItemKindRequestDto[14].deductableFlag === '1' ? 0 : 1
            }
            this.setPremium({
