	...
	<div class="boli">
		<div class="insuranceDetail">
			<div class="detailTop borderBottom">
				<p class="insuranceTitleDes">{{jiashiyuanName}}</p>
				<div class="insurancePrice">
					<p class="inPriceDropDown">
						<select class='select' v-model='jiashiyuanValue' v-on:change='boliShows()'>
							<option v-for='item in jiashiyuan'>{{item.name}}</option>
						</select>
					</p>
				</div>
				<p class="statusWarp">
		...
		<div class="insuranceDetail boli" v-show='jiashiShow'>
			<div class="detailTop borderBottom">
...
import {confirm} from 'common/js/mixin.js'
    export default {
        ...
        methods: {
            ...
            boliShows () {
                let value = this.jiashiyuanValue
                if (value === '投保') {
                    this.jiashiShow = true
                    this.setPremium({
                        field1: 'kinds#2',
                        field2: 'amount',
                        value: ''
            ...
            yesStatus (index) {
                let yesOrno = this.jiashiyuanValue
                if (yesOrno === '投保') {
                    this.Ind = index
            ...
            setValue () {
                if (this.duCarPremium.kinds[2]) {
                    this.jiashiyuanValue = this.duCarPremium.kinds[2].amount ? '不投保' : '投保'
                    this.Ind = this.duCarPremium.kinds[2].model === 'Y' ? 0 : 1
                    this.num = this.duCarPremium.kinds[2].quantity
                    this.siglePrice = this.duCarPremium.kinds[2].unitAmount
                    this.setPremium({
                        field1: 'kinds#2',
                        field2: 'kindCode',
                        value: 'D3'
