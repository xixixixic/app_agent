<template>
	<div class="insuranceDetail">
		<div class="detailTop borderBottom">
			<p class="insuranceTitleDes">{{quanchedaoqiangName}}</p>
			<div class="insurancePrice">
				<p class="inPriceDropDown">
					<select class='select' v-model='quanchedaoqiangValue' v-on:change='setOption'>
						<option v-for='item in quanchedaoqiang'>{{item.name}}</option>
					    <!--<option>投保</option>-->
					</select>
				</p>
			</div>
...
export default {
    ...
    data () {
        return {
            ...
            quanchedaoqiangValue: '不投保',
            status: [
                 {value: '是'},
                 {value: '否'}
                ],
            Ind: 1,
            quanchedaoqiangName: '全车盗抢保险'
        }
    },
    created () {
        this.setValue()
    ...
    methods: {
        ...
        yesStatus (index) {
            let yesOrno = this.quanchedaoqiangValue
            if (yesOrno === '投保') {
                this.Ind = index
        ...
        setValue () {
            if (this.duCarPremium.kinds[4]) {
                this.quanchedaoqiangValue = this.duCarPremium.kinds[4].amount ? '不投保' : '投保'
                this.Ind = this.duCarPremium.kinds[4].model === 'Y' ? 0 : 1
            }
            this.setPremium({
                    field1: 'kinds#4',
                    field2: 'kindCode',
            ...
            this.setPremium({
                    field1: 'kinds#4',
                    field2: 'kindName',
                    value: this.quanchedaoqiangName
               })
            this.setPremium({
                    field1: 'kinds#4',
                    field2: 'model',
        ...
        setOption () {
           let value = this.quanchedaoqiangValue
            if (value === '投保') {
                this.setPremium({
                    field1: 'kinds#4',
                    field2: 'amount',
