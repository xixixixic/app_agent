...
export default {
   ...
   created () {
    ...
    methods: {
        ...
        yesStatus (index) {
            let yesOrno = this.fadongjiValue
            if (yesOrno === '投保') {
                this.Ind = index
        ...
        setValue () {
            // this.fadongjiValue = this.duCarPremium.kinds[0].amount
            if (this.duCarPremium.kinds[11]) {
                this.fadongjiValue = this.duCarPremium.kinds[11].amount ? '不投保' : '投保'
                this.Ind = this.duCarPremium.kinds[11].model === 'Y' ? 0 : 1
            }
            this.setPremium({
                    field1: 'kinds#11',
                    field2: 'kindCode',
