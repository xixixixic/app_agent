<template>
    <div class="detail">
        <div class="info">
            <p class="desTitle" ref='nameTitle'>办公电话</p>
            <p class="placeholders">
                <input type="text" placeholder="请填写办公电话" class="placeholdersInput" v-model="values" @blur='verValue'/>
            </p>
        </div>
...
import {RegTelePhone} from 'common/js/iscardID-util.js'
export default {
    props: {
        Info: {
            type: Object,
    ...
    methods: {
        setDefalut () {
            this.values = this.Info.defaultTelephone
        },
        verValue () {
            if (RegTelePhone(this.values) === false) {
                this.wrongTip = '办公电话'
                this.values = ''               
            } else {
                this.wrongTip = ''
