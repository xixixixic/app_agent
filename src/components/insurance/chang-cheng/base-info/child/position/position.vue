...
<div class="warp">
    <div class="detail">
      <div class="info lastInfo">
        ...
        <p class="placeholders" :class="{'placeholdersAdd': positionValue.length > 14}" @click='choosePosition' v-show='!this.changChengBaseInfo.backStatus'>
        ...
        <p class="placeholders" v-show='this.changChengBaseInfo.backStatus'>
          <span id='tianCarAddress' class="normal active">{{positionValue}}</span>
        </p>
      </div>
    </div>
    <section class="showChose" v-show="showChose">
      <section class="address">
        <section class="bigTitle title">
          <h4>选择职业</h4>
...
import {mapGetters, mapMutations} from 'vuex'
    export default {
      ...
      computed: {
            ...
            valueLength () {
              // :class="'placeholdersAdd': this.valueLength > 14"
              console.log(this.positionValue.length)
              console.log('this.positionValue.length')
              return this.positionValue.length
            },
            ...mapGetters([
              'changChengBaseInfo'
            ])
      ...
      methods: {
        ...
        setSame () {
            this.positionValue = this.changChengBaseInfo.applicant.jobName
        },
        clearSame () {
            this.positionValue = this.changChengBaseInfo.insured.jobName === '' ? '请选择职业名称' : this.changChengBaseInfo.insured.jobName
        },
        choseAdd: function () {
          this.showChose = true
        },
