...
<div>
    <div class="mainInfoLi">
        ...
        <span class="spanRight">
            <input type="text" placeholder='请填写保额' v-model='money' @blur='setMoney'>
        </span>
    </div>
    <div class='wrongTip' v-show='wrongTipStatus'>
        <p class='wrongWord'>投保规则错误</p>
        <p class='wrongWord'>1.最低为{{this.changChengPolicyInfo.minMoney}}元且增加部分为{{this.changChengPolicyInfo.addRate}}的整数倍</p>
    </div>
...
export default {
    ...
    methods: {
        setDefault () {
            this.money = this.changChengPolicyInfo.mainPolicy.amount
        },
        setMoney () {
            ...
            } else if (this.changChengPolicyInfo.mainPolicy.getMoneyYear === '') {
                this.setChangChengPolicyInfo({
                    field1: this.mainDetailPilicyType,
                    field2: 'amount',
                    value: this.money
                })
                this.showConfirm('请先选择祝寿金领取年龄')
            } else {
                let money = this.money
                console.log('policyMoney')
                console.log(money)
