<template>
    <div class="applicantInfo">
		...
		<div class='insuredWarp'>
			...
			<resident-type :Info='Info'></resident-type>
			<name :Info='Info'></name>
			<sex :Info='Info'></sex>
			...
			<source-of-income :Info='Info'></source-of-income>
			<FamilyIncome :Info='Info'></FamilyIncome>
			<phone :Info='Info'></phone>
			<email :Info='Info'></email>
			<zip-code :Info='Info'></zip-code>
			<grp-name :Info='Info'></grp-name>
			<contact-address :Info='Info'></contact-address>
			<approach :Info='Info'></approach>
			<purpose :Info='Info'></purpose>
			<div class="detailTitle">
...
import FamilyIncome from 'components/insurance/guang-da/common/child/familyIncome/familyIncome.vue'
...
import ResidentType from 'components/insurance/guang-da/common/child/residentType/residentType.vue'
import ContactAddress from 'components/insurance/guang-da/common/child/contact-address/contact-address.vue'
import sourceOfIncome from 'components/insurance/guang-da/common/child/sourceofIncome/sourceofIncome.vue'
import TaxPeopleInfo from 'components/insurance/guang-da/common/child/tax-people-info/tax-people-info.vue'
import Approach from 'components/insurance/guang-da/common/child/approach/approach.vue'
import Purpose from 'components/insurance/guang-da/common/child/purpose/purpose.vue'
...
export default {
	...
	computed: {
		Info () {
			return {
				type: 'appnt',
				defaultName: this.guangdaTrail.policy.custInfo.appnt.name, // 姓名  1
				defaultSex: this.guangdaTrail.policy.custInfo.appnt.appntSex, // 性别  1 
				defaultIdType: this.guangdaTrail.policy.custInfo.appnt.idType, // 证件类型  1
				defaultIdNo: this.guangdaTrail.policy.custInfo.appnt.idNo, // 证件号码  1 
				defaultBirthday: this.guangdaTrail.policy.custInfo.appnt.appntBirthday, // 生日 1 
				defaultappOccupation: this.guangdaTrail.policy.custInfo.appnt.appOccupation, // 职业名称 1
				defaultOccupationCode: this.guangdaTrail.policy.custInfo.appnt.occupationCode, // 职业代码 1
				defaultOccupationType: this.guangdaTrail.policy.custInfo.appnt.occupationType, // 职业类别 1
				defaultIncome: this.guangdaTrail.policy.custInfo.appnt.salary, // 年收入 1
				defaultNation: this.guangdaTrail.policy.custInfo.appnt.nativePlace, // 国籍 2
				defaultMarrige: this.guangdaTrail.policy.custInfo.appnt.marriage, // 婚姻状况 2 
				defaultMobile: this.guangdaTrail.policy.custInfo.appnt.mobile, // 手机 2
				defaultEmail: this.guangdaTrail.policy.custInfo.appnt.email, // 邮箱 2 
				defaultZipCode: this.guangdaTrail.policy.custInfo.appnt.zipCode, // 邮编 2 
				defaultFamilyIncome: this.guangdaTrail.policy.custInfo.appnt.familySalay, // 家庭年收入 2
				defaultHeight: this.guangdaTrail.policy.custInfo.appnt.appntStature, // 身高 2
				defaultWeight: this.guangdaTrail.policy.custInfo.appnt.appntAvoirdupois, // 体重 2
				defaultGrpName: this.guangdaTrail.policy.custInfo.appnt.grpName, // 工作单位 2
				defaultIdExpDate: this.guangdaTrail.policy.custInfo.appnt.idExpDate, // 证件有效期 2
				defaultCommunity: this.guangdaPolicy.appntCommunity, // 社区 2
				defaultStreet: this.guangdaPolicy.appntStree, // 街道 2
				defaultTotalAssets: this.guangdaTrail.policy.policyInfo.totalAssets, // 投保人总资产 2
				defaultResidentType: this.guangdaTrail.policy.custInfo.appnt.residentType, // 居民类型 2
				defaultSourceOfIncome: this.guangdaTrail.policy.policyInfo.sourceOfIncome, // 个人年收入来源 2
				defaultPostalAddress: this.guangdaPolicy.appntAlladress, // 联系地址 2
				defaultApproachDes: this.guangdaTrail.policy.policyInfo.agentReport.approachDes, // 投保途径描述 2
				defaultApproachList: this.guangdaTrail.policy.policyInfo.agentReport.approachList, // 投保途径选择 2
				defaultPurposeDes: this.guangdaTrail.policy.policyInfo.agentReport.purposeDes, // 投保目的描述 2
				defaultPurposeList: this.guangdaTrail.policy.policyInfo.agentReport.purposeList, // 投保目的选择 2
				defaultRelationId: this.guangdaPremiumRisk.insured.relationId, // 与投保人关系
				defaultTaxIdentity: this.guangdaTrail.policy.custInfo.appnt.appntTaxInfo.taxIdentity,  // 税收居民身份
				defalutTin: this.guangdaTrail.policy.custInfo.appnt.appntTaxInfo.taxInfoList.taxInfo[0].tin // 纳税人编号
			}
		},
		...mapGetters([
			'guangdaTrail',
		...
		validate (obj) {
			...
			for(let field in obj) {
				if (field === 'defalutTin' && obj[field] === '') {
					...
					flag = false
				} else if (field === 'defaultSourceOfIncome' && obj[field] === '') {
					flag = false
				} else if (field === 'defaultStreet' && obj[field] === '') {
					flag = false
				} else if (field === 'defaultTaxIdentity' && obj[field] === '') {
					flag = false
				} else if (field === 'defaultTotalAssets' && obj[field] === '') {
					flag = false
					...
					flag = false
				} else if (field === 'approachList') {
					if (obj[field].length !== 0) {
						obj[field].map((item) => {
							if (item.approach = '6' && obj['approachDes'] === '') {
								flag = false
							}
						})
					} else {
				...
				} else if (field === 'purposeList') {
					if (obj[field].length !== 0) {
						obj[field].map((item) => {
							if (item.purpose = '4' && obj['purposeDes'] === '') {
								flag = false
							}
						})
					} else {
